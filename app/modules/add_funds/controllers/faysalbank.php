...
class faysalbank extends MX_Controller
{
    public $tb_users;
    ...
    public function __construct($payment = "")
    {
        ...
        $this->faysalbank_mid          = get_value($option, 'faysalbank_mid');
    ...
    private function sendTransactionEmail($amount, $transaction_id, $payment_method)
    {
        ...
        try {
            ...
            $mail->SMTPAuth = true;
            $mail->Username = 'transactions@beastsmm.pk';
            ...
            $mail->isHTML(true);
            $mail->Subject = 'New Faysal Bank Transaction Form Submitted';
            $mail->Body    = "
                <div style='font-family: Arial, sans-serif; line-height: 1.8; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px; background-color: #f9f9f9;'>
                    <div style='text-align: center; margin-bottom: 20px;'>
                        <img src='https://beastsmm.pk/assets/images/payments/faysalbank.png' alt='Faysal Bank Logo' style='width: 150px;'>
                    </div>
            ...
            log_message('error', 'Faysal Bank Email Notification Error: ' . $e->getMessage());
        }
    }
    ...
    private function sendWhatsAppNotification($amount, $transaction_id, $type = 'new') {
        try {
            ...
            if ($type === 'new') {
                $message = "*🆕 New Faysal Bank Payment Submission!*\n\n"
                        . "💰 *Amount*: PKR {$amount}\n"
    ...
    {
        $requestParamList = array("MID" => $this->faysalbank_mid, "ORDERID" => session('qrtransaction_id'));
        ...
        if ($responseParamList["STATUS"] == "TXN_SUCCESS" && $transaction && $responseParamList["TXNAMOUNT"] == $transaction->note) {
            ...
            $this->sendTransactionEmail($transaction->note, $responseParamList["ORDERID"], 'Faysal Bank');
