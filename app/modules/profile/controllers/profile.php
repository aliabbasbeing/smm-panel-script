...
class profile extends MX_Controller {
	...
	public function ajax_update_whatsapp_number($ids = '') {
		$id = session('uid'); // Get the user ID from the session
		$whatsapp_number = post('whatsapp_number'); // Get the posted WhatsApp number
		
		// Validate WhatsApp number format
		if ($whatsapp_number == '') {
			ms(array(
				'status'  => 'error',
				'message' => lang("whatsapp_number_is_required"),
			));
		}
	
		// Check if the number format is valid (basic regex for phone numbers)
		if (!preg_match("/^(\+?\d{1,4}[\s-]?)?(\(?\d{1,4}?\)?[\s-]?\d{1,4}[\s-]?\d{1,4})$/", $whatsapp_number)) {
			ms(array(
				'status'  => 'error',
		...
		if ($id != '') {
			// Fetch the user data from the general_users table by user ID
			$checkUser = $this->model->get('id, whatsapp_number', 'general_users', "`id` = '{$id}'");
			...
			// Update the WhatsApp number in the general_users table
			if ($this->db->update('general_users', $data, "id ='{$id}'")) {
				ms(array(
					'status'  => 'success',
					'message' => lang('Update_successfully'),
				));
